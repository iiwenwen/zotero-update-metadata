...
如果 CNB CLI 不可用、登录态缺失或 Issue 创建失败，允许创建本地 provisional task 做诊断、计划、测试准备或 branch prep，但不得 commit、push、创建 PR 或关闭 Issue，直到 CNB 同步成功。必须记录：
...
```text
READY -> CLAIMED -> IN_PROGRESS -> VERIFYING -> REVIEWING -> PR_OPEN -> MERGED -> DONE
```
...
`PR_OPEN` 表示 Agent 工作已完成并等待人类 review/merge。watchdog 不得把 `PR_OPEN` 任务重新选为可执行任务，除非 PR 更新、冲突、CI 失败、review comment 或用户明确要求继续处理。
...
- 目标模糊的任务
- 缺少验收标准的任务
- 状态为 `PR_OPEN` 且没有 PR 更新、冲突、CI 失败、review comment 或用户继续处理要求的任务
- 涉及超过 4 个核心实体的任务
- 需要高风险操作的任务
- 需要人类产品决策的任务
...
- P0/P1 总是阻塞 `PASS`、Issue closure 和 PR 创建
...
### 10.1 PR_SELF_REVIEW：PR 前自审

创建 PR 或准备 PR handoff 前，必须完成自审。
...
`.ai/STATE.md`、`.ai/QUEUE.md`、`.ai/runs/`、`.ai/memory/` 是 `.ai local runtime state`，默认不进 PR。允许这些文件在本机保持 dirty；PR 前必须确认它们没有 staged，并在最终输出说明它们是 local runtime state。
...
5. 如需 PR，PR_SELF_REVIEW 已通过
6. 受控 git checkpoint commit 已完成

默认分支工作流下，Agent 完成验证、自审、commit、push 并创建 PR 后，任务状态为 `PR_OPEN`。`PR_OPEN` 是合法交付终态，表示 Agent work complete, awaiting human merge。此时：

- 可以输出 `PASS`
- 本地队列状态保持 `PR_OPEN`
- CNB Issue 保持 open，直到 PR merged 或用户明确要求关闭
- watchdog 不得重复执行该任务，除非 PR 更新、冲突、CI 失败、review comment 或用户明确要求继续处理
...
4. VERIFY、REVIEW、PR_SELF_REVIEW 全部通过后，创建 PR。
5. 除非用户明确要求并确认，不得由 Agent 自行合并 PR。
6. PR 创建后将本地队列状态设为 `PR_OPEN`；只有 PR 合并后，才关闭对应 CNB Issue；如果用户明确要求先关闭，必须在 run 记录原因。

允许例外：

- 用户明确要求直接在 `main` 修复。
- 紧急恢复或仓库维护要求直接动 `main`，且用户确认。
- no-change review/advice 不创建分支、不创建 PR。
...
Commit: <hash>
PR: <url or N/A>
Issue: <closed | open until PR merge | N/A>
Notes: <none 或剩余风险>
```

禁止只输出裸 `PASS`。`PASS` 后必须说明当前任务完成程度、验证结果、真实 commit hash、PR/Issue 状态和遗留风险。默认分支工作流下，`State: PR_OPEN` 表示 Agent work complete, awaiting human merge。

...
本 skill 的写入范围仅限本地忽略的 `.ai/**`：

- 主要读写 `.ai/memory/**` 下的本地 AI 记忆。
- 只有当前自主流程已经使用 `.ai/STATE.md` 或 `.ai/tasks/<task-id>.md` 时，才可以把本轮创建、更新、跳过或 supersede 的记忆条目 ID 追加到这些本地状态/任务备注中。

它不修改产品代码、spec、评审发现、Issue、PR、提交、`.codex`、CNB 状态或真实 Zotero 数据。`.ai/**` 必须保持 local-only，绝不能被 stage 或 commit。
...
写入 `.ai/memory/**` 前必须重新读取目标文件的最新内容，不能复用开工前的旧快照来分配 ID 或判断去重。
...
2. 再次执行去重检查。
3. 扫描同日期、同前缀已有 ID。
4. 选择当前最大数字后缀加 1 的下一个可用 ID。
5. 如果目标 ID 已存在，重新扫描并选择下一个可用 ID。
6. 追加写入后，再读取文件确认条目存在且 ID 唯一。

如果无法证明当前没有并发写入者，或写入后发现 ID 冲突、重复条目、部分写入，就停止写入并输出：

```text
FLYWHEEL_SKIPPED: concurrent writer risk
```
